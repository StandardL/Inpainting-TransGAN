...
class ImageDataset(object):
    def __init__(self, args, cur_img_size=None, bs=None):
        ...
        if args.dataset.lower() == 'cifar10':
            ...
            # val_size = int(0.1 * len(celeba_dataset))
            # test_size = len(celeba_dataset) - train_size - val_size
            # torch.manual_seed(args.seed)
            # train_dataset, val_dataset, test_dataset = torch.utils.data.random_split(celeba_dataset,
            ...
            self.train = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.dis_batch_size, shuffle=(train_sampler is None),
                num_workers=args.num_workers, pin_memory=False, drop_last=True, sampler=train_sampler)

            self.valid = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.dis_batch_size, shuffle=False,
                num_workers=args.num_workers, pin_memory=False, drop_last=True, sampler=val_sampler)

            self.test = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.dis_batch_size, shuffle=False,
                num_workers=args.num_workers, pin_memory=False, drop_last=True, sampler=test_sampler)

        # Add place365 dataset
            ...
            place_dataset = Dt(root=args.data_path, transform=transform)

            # train_dataset = Dt(root=args.data_path, transform=transform)
            # val_dataset = Dt(root=args.data_path, transform=transform)

            # Split train, val, test
            train_size = int(0.7 * len(place_dataset))
            val_size = int(0.1 * len(place_dataset))
            test_size = len(place_dataset) - train_size - val_size
            torch.manual_seed(args.seed)
            train_dataset, val_dataset, test_dataset = torch.utils.data.random_split(place_dataset,
                                                                                     [train_size, val_size, test_size])
            dist.init_process_group(backend=args.dist_backend, world_size=args.world_size, rank=args.rank,
                                    init_method=args.dist_url)
            train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
