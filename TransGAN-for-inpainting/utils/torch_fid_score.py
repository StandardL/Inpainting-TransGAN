    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(n_batches) ):
            ...
            if args.train_type == 'inpainting':
                gen_imgs = gen_net(z, mask, images, 200)
            else:
...
def calculate_activation_statistics(args, gen_net, model, batch_size=50,
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations(args, gen_net, model, batch_size, dims, cuda, verbose)
...
def calculate_fid_given_paths_torch(args, gen_net, path, require_grad=False, gen_batch_size=1, batch_size=1, cuda=True,
    ...
    with _get_no_grad_ctx_mgr(require_grad=require_grad):
        ...
        fid_value = torch_calculate_frechet_distance(m1.to("cuda:0"), s1.to("cuda:0"),
...
def get_fid(args, fid_stat, epoch, gen_net, num_img, gen_batch_size, val_batch_size, writer_dict=None, cls_idx=None):
    ...
    with torch.no_grad():
        ...
        #     else:
        #         if args.train_type == 'inpainting':
        #             gen_imgs = gen_net(z, mask, images, epoch)
        #         else:
        #             gen_imgs = gen_net(z, epoch)
