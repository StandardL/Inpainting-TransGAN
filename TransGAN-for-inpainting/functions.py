...
from  utils.torch_fid_score_using_pytorch_fid import get_fid as get_fid_using_pytorch

from utils.torch_inception_score import torch_get_inception_score
# python exps/dist1_new_church256.py --node 0022 --rank 0sample

logger = logging.getLogger(__name__)

def gaussian_blur_mask(image, radius):
    """
    Apply Gaussian blur to the image mask.
    :param image: The image mask.
    :param radius: The radius of the blur.
    :return: The blurred image mask.
    """
...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    for iter_idx, (imgs) in enumerate(tqdm(train_loader)):
        ...
        if args.train_type == 'inpainting':
            # Sample mask for generator input
            ...
            z_mask = z_mask[0]
            avgPool = nn.AdaptiveAvgPool2d((args.latent_dim, args.latent_dim))
            z_mask = avgPool(z_mask.unsqueeze(0))
            z_mask = z_mask[0]
            # scale up
            ...
            mask = mask.unsqueeze(0).cuda() + torch.zeros(real_imgs.shape).cuda(args.gpu)
            fake_imgs = gen_net(masked_z.cuda(args.gpu), mask.cuda(args.gpu), real_imgs, epoch).detach()
            # print(f"z.shape: {z.shape} masks.shape: {masks.shape} images_masked.shape: {images_masked.shape}")
            # print(masks)
            # print(images_masked)
            # print(z)
        else:
        ...
        if global_steps % (args.n_critic * args.accumulated_times) == 0:
            ...
            for accumulated_idx in range(args.g_accumulated_times):
                if args.train_type == 'inpainting':
                    # add mask
                    # mask = cv2.resize(cv2.imread(args.mask_path), (256, 256))
                    # mask = cv2.cvtColor(mask, cv2.COLOR_BGR2RGB)
                    # mask = (ToTensor()(mask)).unsqueeze(0)
                    gen_z = torch.cuda.FloatTensor(np.random.normal(0, 1, (args.gen_batch_size, args.latent_dim)))
                    # mask = (gen_z*(1-mask).float()) + mask
                    # images, masks = images.cuda(), masks.cuda()
                    # images_masked = (images * (1 - masks).float()) + masks
                    z_mask = mask = masks[np.random.randint(0, masks.shape[0])]
                    z_mask = z_mask[0]
                    ...
                    masked_z = torch.mm(gen_z, z_mask)
                    mask = mask.unsqueeze(0).cuda() + torch.zeros(real_imgs.shape).cuda(args.gpu)
                    gen_imgs = gen_net(masked_z.cuda(args.gpu), mask.cuda(args.gpu), real_imgs, epoch)
                else:
                    gen_z = torch.cuda.FloatTensor(np.random.normal(0, 1, (args.gen_batch_size, args.latent_dim)))
                ...
                if args.loss == "lsgan":
                    ...
                    z_random1, z_random2 = images_masked[:args.gen_batch_size // 2], images_masked[args.gen_batch_size // 2:]
...
def validate(args, fixed_z, fid_stat, epoch, gen_net: nn.Module, writer_dict, clean_dir=True):
    ...
    if args.rank == '0':
        mean, std = torch_get_inception_score(img_list)
        #mean, std = 0, 0
    else:
        mean, std = 0, 0
...
def validate_mask(args, fixed_z, mask, real, fid_stat, epoch, gen_net: nn.Module, writer_dict, clean_dir=True):
    writer = writer_dict['writer']
    global_steps = writer_dict['valid_global_steps']
    ...
    #     generate images
    with torch.no_grad():
        sample_imgs = gen_net(fixed_z, mask, real, epoch)
        img_grid = make_grid(sample_imgs, nrow=5, normalize=True, scale_each=True)

    # get fid and inception score
        fid_buffer_dir = os.path.join(args.path_helper['sample_path'])
        os.makedirs(fid_buffer_dir, exist_ok=True) if args.gpu == 0 else 0
        dataset = datasets_shoucuo.ImageDataset(args, cur_img_size=8)
        valid_loader = dataset.valid
        eval_iter = args.num_eval_imgs // args.eval_batch_size
        img_list = list()
        for iter_idx, (imgs) in enumerate(tqdm(valid_loader, desc="sample images")):
            real_imgs = imgs.type(torch.cuda.FloatTensor).cuda(args.gpu, non_blocking=True)
            z = torch.cuda.FloatTensor(np.random.normal(0, 1, (args.eval_batch_size, args.latent_dim)))
            images, masks, _ = next(create_loader(args))
            images, masks = images.cuda(), masks.cuda()
            ...
            z_mask = ToTensor()(z_mask_pil).squeeze(0).cuda()
            z = torch.mm(z, z_mask)
            mask = mask.unsqueeze(0) + torch.zeros(real_imgs.shape).cuda(args.gpu)
            # Generate a batch of images
            gen_imgs = gen_net(z, mask, real_imgs, epoch).mul_(127.5).add_(127.5).clamp_(0.0, 255.0).permute(0, 2, 3, 1).to('cpu',
                                                                                                          torch.uint8).numpy()
            for img_idx, img in enumerate(gen_imgs):
                file_name = os.path.join(fid_buffer_dir, f'iter{iter_idx}_b{img_idx}.png')
                imsave(file_name, img)
    ...
    print('=> calculate fid score') if args.rank == 0 else 0
    if args.rank == 0:
        fid_score = get_fid_using_pytorch(args, fid_stat, epoch, gen_net, args.num_eval_imgs, args.gen_batch_size,
                            args.eval_batch_size, writer_dict=writer_dict, cls_idx=None)
    else:
        fid_score = 10000
    # fid_score = 10000
